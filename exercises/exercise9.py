...
def suma_cubo_pares_for(numeros: Iterable[int]) -> int:
    ...
    for x in numeros:
        numeros_al_cubo.append(x**3)
    
    for x in numeros_al_cubo:
        if x%2 == 0:
            suma = suma + x
...
numeros_al_cubo = [1,2,3] # Completar
cubos = list(map(lambda x: x ** 2, numeros_al_cubo))
print(cubos)
...
numeros_al_cubo_pares =  [1,2,3] # Completar
cubos = list(map(lambda x: x ** 3, numeros_al_cubo))
pares = list(filter(lambda x: x % 2 == 0, cubos))
print(pares)
...
numeros_al_cubo_pares = [8, 64, 216]
